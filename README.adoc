...
This booster expose a simple `api/greetings/` service that produce a response based on a configuration property in JSON format of the form `xxxx`.
...
. Follow the on-screen instructions to create a new *Configmap Level 0* project using the *Fuse* runtime.
