...
. Above the entry for the `fuse-configmap-booster` application on the `Overview` page, locate the URL of the form `http://fuse-configmap-booster-MY_PROJECT_NAME.OPENSHIFT_IP_ADDR.nip.io`.

. Click on the URL to access the greetings service application and then follow the instructions on that page.

== Running the booster on OpenShift Online
To deploy the configmap booster directly to OpenShift Online when you create the project at link:{launchURL}[]:
